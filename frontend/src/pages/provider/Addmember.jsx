...
export default function ProviderTeam() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
            <main className="max-w-7xl mx-auto sm:px-6 lg:px-8 py-8 pt-0 md:pt-6">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">

                    <div className="lg:col-span-4">
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm lg:sticky lg:top-8">
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        <div className="w-full bg-white rounded-2xl border border-slate-200 shadow-sm p-6 overflow-hidden relative">
                            <div className="flex flex-col sm:flex-row items-center justify-start gap-8 divide-y sm:divide-y-0 sm:divide-x divide-slate-100">
                                <div className="w-full sm:w-auto pr-8 py-2 sm:py-0">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 block">Team Personnel</span>
                                ...
                                <div className="w-full sm:w-auto sm:px-8 py-4 sm:py-0">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 block">Team Capacity</span>
                            ...
                            <div className="max-h-[600px] overflow-y-auto">
                                {loading ? (
                                    ...
                                    <div className="divide-y divide-slate-100">
                                        {members.map((member) => (
                                            <div key={member._id} className="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50 transition-colors group">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="min-w-0">
                                                        <h4 className="font-bold text-sm leading-none truncate">{member.fullname}</h4>
                                                        <span className="text-indigo-600 text-[11px] font-bold uppercase tracking-wider">{member.role}</span>
                                                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-slate-400">
                                                            <span className="flex items-center gap-1 text-[11px] font-medium whitespace-nowrap"><Phone size={10} /> {member.phonenumber}</span>
                                                            <span className="flex items-center gap-1 text-[11px] font-medium truncate"><Mail size={10} /> {member.email || 'N/A'}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <button
                                                    ...
                                                    className="self-end sm:self-center p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors"
                                                >
                                                    <Trash2 size={18} />
                                                </button>
                                            </div>
